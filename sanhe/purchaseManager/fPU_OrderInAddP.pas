...
procedure TfPU_OrderInAdd.cxGrid1DBTableView1EditChanged(
...
desc : String;
begin
    column := cxGrid1DBTableView1.Controller.FocusedColumnIndex;
    if column = 4 then
    begin
        row := cxGrid1DBTableView1.DataController.FocusedRecordIndex;
        buyCount := strToFloat(cxGrid1DBTableView1.ViewData.Records[Row].Values[3]);
        hadInCount := strToFloat(cxGrid1DBTableView1.ViewData.Records[Row].Values[4]);
        inString := cxGrid1DBTableView1.Controller.EditingController.Edit.EditingValue;
        if inString = '' then
           inCount := 0
         else
            inCount := strToFloat(inString);
       remaining := buyCount - hadInCount -inCount;

       if remaining < 0 then
       begin
         desc := cxGrid1DBTableView1.ViewData.Records[row].Values[2];
         showmessage(''' '+desc + ''' 入库数量不能大于采购数量');
         exit;
       end;
    end;
end;
