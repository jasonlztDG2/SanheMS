...
type
  ...
  public
    { Public declarations }
    procedure getOrderdtId(id : String;productId : String);
...
procedure TFPU_OrderInList.getOrderdtId(id : String;productId : String);
begin
    orderdtListID.Sorted := True;  //需要先指定排序
    orderdtListID.Duplicates := dupIgnore;  //如有重复值则放弃
    orderdtListID.Add(id+'='+id);

    productListID.Sorted := True;  //需要先指定排序
    productListID.Duplicates := dupIgnore;  //如有重复值则放弃
    productListID.Add(productId+'='+productId);
end;
   ...
   begin
       for I := 0 to orderdtListID.Count - 1 do
        begin
             ...
             productList.Sorted := True;  //需要先指定排序
             productList.Duplicates := dupIgnore;  //如有重复值则放弃
             productList.add(productListID[i]);
         end;
  end;
...
begin
    ...
    fPU_OrderInAdd.partnersId := cxGridDBTableView1.DataController.Values[row,2];
    fPU_OrderInAdd.Edit2.Text := cxGridDBTableView1.DataController.Values[row,1];
    duPub.getDobInData(duPub.tbl_pu_orderDt,orderDataList,'pu_orderDt','id');

    self.tbl_pu_order.Close;
    duPub.tbl_pu_orderDtProduct.Close;
    self.Close;
end;

procedure TfPU_OrderInList.Button3Click(Sender: TObject);
begin
     self.tbl_pu_order.Close;
    duPub.tbl_pu_orderDtProduct.Close;
    self.Close;
  ...
  inherited;
   ...
   begin
       ...
       getOrderdtId(duPub.tbl_pu_orderdtProduct.FieldByName('id').AsString,
                  duPub.tbl_pu_orderdtProduct.FieldByName('productID').AsString);
   end;
...
begin
     ...
     productListID := TStringList.Create;
